...
BQuad WordToBQuad(WORD v) {
    return {B(v), B(v >> 8), B(v >> 16), B(v >> 24)};
}

DWORD BQuad2ToDword(BQuad2 v) {
    return (
        DWORD(v.a[0]) | DWORD(v.a[1]) << 8 | DWORD(v.a[2]) << 16 | DWORD(v.a[3]) << 24 |
        DWORD(v.a[0]) << 32 | DWORD(v.a[1]) << 40 | DWORD(v.a[2]) << 48 | DWORD(v.a[3]) << 56
    );
}

BQuad2 DwordToBQuad2(DWORD v) {
...
__m256i BQuad8ToM256(BQuad8 v) {
    return {
        static_cast<long long>(BQuad2ToDword(BQuad2(v.a, v.b))),
