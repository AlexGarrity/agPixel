...
uint32_t dPixelArray::GetSize() const { return m_width * m_height; }
...
void PerformOperationInPlace(dPixelArray &a, dPixelArray &b,
                             DQuad fn(DQuad, DQuad)) {
...
void PerformOperationInPlace(dPixelArray &a, DQuad &qB, DQuad f1(DQuad, DQuad),
                             DQuad f2(DQuad, DQuad)) {
  uint8_t remainder = a.GetSize() % 2;
  DQuad qA;
  for (auto i = 0; i < (a.GetSize() - remainder); i += 2) {
    qA = PixelToDQuad(a.GetPixel(i));
