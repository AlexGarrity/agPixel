...
uint32_t PixelArray::GetSize() const {
...
void PerformOperationInPlace(PixelArray &a, PixelArray &b, BQuad8 f1(BQuad8, BQuad8), BQuad f2(BQuad, BQuad)) {
...
void PerformOperationInPlace(PixelArray &a, BQuad8 &qB, BQuad8 f1(BQuad8, BQuad8), BQuad f2(BQuad, BQuad)) {
        uint8_t remainder = a.GetSize() % 8;
        BQuad8 qA;
        for (auto i = 0; i < (a.GetSize() - remainder); i+=8) {
                qA.a = PixelToBQuad(a.GetPixel(i+0));
